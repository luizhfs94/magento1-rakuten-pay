...
trait Parameter
{
    ...
    public function addParameter()
    {
        $this->parameter = InitializeObject::Initialize(
            $this->parameter,
            new \RakutenPay\Resources\Factory\Request\Parameter()
        );
    ...
    public function setParameter($parameter)
    {
        if (is_array($parameter)) {
            ...
            foreach ($parameter as $key => $parameterItem) {
                if ($parameterItem instanceof \RakutenPay\Domains\Parameter) {
                    $arr[$key] = $parameterItem;
                } else {
                    if (is_array($parameter)) {
                        $arr[$key] = new \RakutenPay\Domains\Parameter($parameterItem);
                    }
                }
            }
            $this->parameter = $arr;
