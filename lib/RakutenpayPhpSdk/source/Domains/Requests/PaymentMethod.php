...
trait PaymentMethod
{
    ...
    public function addPaymentMethod()
    {
        $this->paymentMethod = InitializeObject::Initialize(
            $this->paymentMethod,
            new \RakutenPay\Resources\Factory\Request\PaymentMethod()
        );
    ...
    public function setPaymentMethod($paymentMethod)
    {
        if (is_array($paymentMethod)) {
            ...
            foreach ($paymentMethod as $key => $method) {
                if ($method instanceof \RakutenPay\Domains\PaymentMethod) {
                    $arr[$key] = $method;
                } else {
                    if (is_array($paymentMethod)) {
                        $arr[$key] = new \RakutenPay\Domains\PaymentMethod($method);
                    }
                }
            }
