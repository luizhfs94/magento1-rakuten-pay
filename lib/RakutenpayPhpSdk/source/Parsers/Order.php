...
/**
 * Class Moede
 * @package RakutenPay\Parsers\Order
 */
trait Order
{
    /**
    ...
    public static function getData($request, $properties)
    {
        ...
        if (!is_null($request->getShipping()) and !is_null($request->getShipping()->getCost())) {
            $shipping_cost = floatval(\RakutenPay\Helpers\Currency::toDecimal($request->getShipping()->getCost()->getCost()));
        }
    ...
    private static function getItem($item, $properties)
    {
        ...
        if ($item->getAmount() != null) {
            $amount = floatval(\RakutenPay\Helpers\Currency::toDecimal($item->getAmount()));
