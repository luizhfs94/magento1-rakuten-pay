...
trait Item
{
    ...
    {
        Logger::info('Processing getData in trait Item.');
        $data = [];
        ...
        if ($request->itemLenght() > 0) {
            ...
            foreach ($items as $key => $value) {
                ...
                if ($items[$key]->getAmount() != null) {
                    $amount = \Rakuten\Connector\Helpers\Currency::toDecimal($items[$key]->getAmount());
                ...
                if ($items[$key]->getShippingCost() != null) {
                    $data[sprintf($properties::ITEM_SHIPPING_COST, $count)] = \Rakuten\Connector\Helpers\Currency::toDecimal(
