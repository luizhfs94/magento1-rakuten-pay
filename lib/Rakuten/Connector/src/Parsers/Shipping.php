...
trait Shipping
{
    ...
    {
        Logger::info('Processing getData in trait Shipping.');
        $data = [];
        // shipping
    ...
    {
        $data = [];
        $data[$properties::SHIPPING_COST] = \Rakuten\Connector\Helpers\Currency::toDecimal(
    ...
    private static function address($request, $properties)
    {
        ...
        if (!is_null($request->getShipping()->getAddress()->getPhone())) {
            $data[$properties::SHIPPING_ADDRESS_PHONE] = $request->getShipping()->getAddress()->getPhone();
        }
        return $data;
    }
}
