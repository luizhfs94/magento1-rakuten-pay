        ...
        if ($request->itemLenght() > 0) {
            ...
            foreach ($items as $key => $value) {
                ...
                if ($items[$key]->getAmount() != null) {
                    $amount = \RakutenConnector\Helpers\Currency::toDecimal($items[$key]->getAmount());
                ...
                if ($items[$key]->getShippingCost() != null) {
                    $data[sprintf($properties::ITEM_SHIPPING_COST, $count)] = \RakutenConnector\Helpers\Currency::toDecimal(
