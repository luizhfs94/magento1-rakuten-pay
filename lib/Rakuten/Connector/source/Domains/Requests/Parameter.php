...
trait Parameter
{
    ...
    public function addParameter()
    {
        $this->parameter = InitializeObject::Initialize(
            $this->parameter,
            new \RakutenConnector\Resources\Factory\Request\Parameter()
        );
    ...
    public function setParameter($parameter)
    {
        if (is_array($parameter)) {
            ...
            foreach ($parameter as $key => $parameterItem) {
                if ($parameterItem instanceof \RakutenConnector\Domains\Parameter) {
                    $arr[$key] = $parameterItem;
                } else {
                    if (is_array($parameter)) {
                        $arr[$key] = new \RakutenConnector\Domains\Parameter($parameterItem);
                    }
                }
            }
            $this->parameter = $arr;
