...
class Rakuten_RakutenPay_Model_InstallmentsMethod extends MethodAbstract
{
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Constructing ModelInstallmentsMethod.');
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing create in ModelInstallmentsMethod.');
        $this->helper = Mage::helper('rakutenpay');
        try {
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing getMaxNoInstallments in ModelInstallmentsMethod.');
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing createInstallments in ModelInstallmentsMethod.');
        $arr = [];
        if (Mage::getStoreConfig('payment/rakutenpay_credit_card/customer_interest') == 1)
        {
            $checkout = \RakutenConnector\Services\Transactions\Checkout::get(
                ['amount' => $amount]
            );
            ...
            foreach($checkout->getCreditCardPayment()->getInstallments() as $customerInterestInstallment) {
                $installment = new RakutenConnector\Domains\Responses\Installment;
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing output in ModelInstallmentsMethod.');
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing formatOutput in ModelInstallmentsMethod.');
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing formatInstallments in ModelInstallmentsMethod.');
        return array(
            'quantity'        => $installment->getQuantity(),
            'amount'          => $installment->getAmount(),
            'totalAmount'     => RakutenConnector\Helpers\Currency::toDecimal($installment->getTotalAmount()),
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing getInstallmentText in ModelInstallmentsMethod.');
        return sprintf(
            "%s x de R$ %.2f %s juros",
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing getInterestFreeText in ModelInstallmentsMethod.');
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing getMaxInstallment in ModelInstallmentsMethod.');
    ...
    {
        \RakutenConnector\Resources\Log\Logger::info('Processing enabled in ModelInstallmentsMethod.');
