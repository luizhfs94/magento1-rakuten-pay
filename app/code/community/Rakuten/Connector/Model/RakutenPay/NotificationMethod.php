...
class Rakuten_Connector_Model_RakutenPay_NotificationMethod extends MethodAbstract
{
    private $helper;
    ...
    {
        \RakutenPay\Resources\Log\Logger::info("Initializing the notification model.", ['service' => 'WEBHOOK']);
    ...
    private function setNotificationUpdateOrder()
    {
        ...
        \RakutenPay\Resources\Log\Logger::info("Processing webhook with transaction: " . $orderId
                    . "; Status: ". $orderStatus . "; Amount: " . $this->amount,
                    ['service' => 'WEBHOOK']);
        if ($orderStatus == false) {
            \RakutenPay\Resources\Log\Logger::error("Cannot process webhook with transaction: " . $transactionCode,
                    ['service' => 'WEBHOOK']);
            return;
        }
        $class = null;
