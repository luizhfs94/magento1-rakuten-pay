...
var Messages = new function() {
    var wrapper = jQuery("#rakutenpay-module-contents");
    var getHtml = function(options) {
        return '<div id="'+ options.id +'" class="rakutenpay-msg rakutenpay-msg-'+options.type+' rakutenpay-msg-'+options.size+'"><'+options.tag+'>' + options.message + '</'+options.tag+'></div>';
    }
    
    var getConfirmHtml = function(options) {
        return '<div id="'+ options.id +'" class="rakutenpay-msg rakutenpay-msg-'+options.type+' rakutenpay-msg-'+options.size+'"><'+options.tag+'>' + options.message + '</'+options.tag+'><hr /><div class="confirm"><button type="button" class="rakutenpay-button rakutenpay-confirm-button" id="accept">Sim</button><button type="button" class="rakutenpay-button rakutenpay-confirm-button" id="reject">Não</button></div></div>'; 
    }

    var remove = function() {
        wrapper.find('.rakutenpay-msg-error, .rakutenpay-msg-success').remove();
    };

    var add = function(message, type) {
        var html = getHtml({
            id: 'rakutenpay-main-message',
            message: message,
            type: type,
            size: 'small',
            tag: 'p'
...
var Modal = new function(){
    ...
    var showLoading = function(msg) {
        if (jQuery('#rakutenpay-loading-message:visible').length > 0) {
            return false;
        }

        var html = Messages.getHtml({
            id: 'rakutenpay-loading-message',
            type: 'loading',
            size: 'medium',
            message: msg,
            tag: 'h3'
...
var Menu = new function() {
        ...
        allItems.click(function(e){
            ...
            if (!animating) {
                ...
                var showNewPage = function() {
                    ...
                    jQuery(".rakutenpay-module-content").removeClass(selectedClass);
                    jQuery("#rakutenpay-module-content-" + id).addClass(selectedClass);
    ...
    var applyGotoConfig = function() {
        jQuery(".rakutenpay-goto-configuration").click(function(){
            jQuery("#menu-item-1").trigger('click');
            jQuery("#rakutenpay-email-input").focus();
        });
    };

    var retractableMenu = function() {
        jQuery("#rakutenpay-module-menu .children").click(function(){
    ...
    var sessionRetractable = function(){
        ...
        if (sessionStorage.getItem('hasOpen') == 'true') {
            jQuery("#rakutenpay-module-menu .children").closest("li").addClass("open");
        }
    };
...
jQuery(document).ready(function () {
    jQuery('#rakutenpay-check-all').click(function(event) {  //on click 
